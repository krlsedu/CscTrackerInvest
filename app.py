...
load_info = LoadInfo(
    remote_repository=remote_repository, http_repository=http_repository
)
...
def investment_fact():
    headers = http_repository.get_headers()
    dumps = json.dumps(
        investment_handler.investment_facts(http_repository.get_json_body(), headers)
    )
...
def get_investment_facts():
    headers = http_repository.get_headers()
    dumps = json.dumps(
        investment_handler.get_investment_facts(
            http_repository.get_args()["ticker"], headers
        )
    )
    return dumps, 200, {"Content-Type": "application/json"}
...
def add_resume_invest_period():
    headers = http_repository.get_headers()
    args = http_repository.get_args()
    args_ = {"args": args, "headers": headers}
...
def save_aplly_stock():
    try:
        headers = http_repository.get_headers()
        dumps = json.dumps(
            investment_handler.save_aplly_stock(
                http_repository.get_json_body(), headers
            )
        )
        return dumps, 200, {"Content-Type": "application/json"}
...
def save_all_aplly_stock():
    try:
        headers = http_repository.get_headers()
        dumps = json.dumps(
            investment_handler.save_all_aplly_stock(
                http_repository.get_json_body(), headers
            )
        )
        return dumps, 200, {"Content-Type": "application/json"}
...
def add_movement():
    headers = http_repository.get_headers()
    dumps = json.dumps(
        investment_handler.add_movement(http_repository.get_json_body(), headers)
    )
    # get_investments()
    return dumps, 200, {"Content-Type": "application/json"}
...
def add_movements():
    headers = http_repository.get_headers()
    dumps = json.dumps(
        investment_handler.add_movements(http_repository.get_json_body(), headers)
    )
    # get_investments()
    return dumps, 200, {"Content-Type": "application/json"}
...
def get_investments():
    try:
        ...
        message = {"text": "Investments update requested", "status": "ok"}
...
def get_investments_tr(args, headers):
    try:
        ...
        Utils.inform_to_client(
            "{}", "investments", headers, "Investments refresh completed"
        )
...
def att_express():
    if (Utils.work_day() and Utils.work_time()) or http_repository.get_headers().get(
        "force"
    ) == "true":
