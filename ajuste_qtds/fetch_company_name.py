...
def fetch_selo_viabilidade(ticker: str) -> str:
    ...
    # Busca o bloco "Selo de Viabilidade" e extrai o nome da classificação dentro dele
    selo_match = re.search(
        r'Selo de Viabilidade.*?<h3[^>]*class="[^"]*text-base[^"]*font-bold[^"]*uppercase[^"]*"[^>]*>(.*?)</h3>',
        html,
        re.IGNORECASE | re.DOTALL,
    )
    if not selo_match:
        selo_match = re.search(
            r'Selo de Viabilidade.*?<h3[^>]*>(.*?)</h3>',
            html,
            re.IGNORECASE | re.DOTALL,
        )

    if selo_match:
        selo_name = selo_match.group(1).strip()
        print(f"Selo de Viabilidade ({ticker}): {selo_name}")
        return selo_name
    else:
        print(f"Selo de Viabilidade não encontrado para {ticker}")
        return ""


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: python fetch_company_name.py <TICKER>")
        print("Exemplo: python fetch_company_name.py AMAR3")
        sys.exit(1)

    ticker = sys.argv[1].upper()
    fetch_selo_viabilidade(ticker)
