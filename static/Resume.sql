with indices as (select s.name                             as name,
                 ...
                 where it.id = s.investment_type_id
                   ...
                   and 'nenhum' <> :indice),
     indices_final as (select (price_fim / price_ini - 1) * 100       as valorizacao_real
...
select name                                                       as name,
       ...
       SIGN(crescimento_percentual_geral) *
       round((power(1 + ABS(crescimento_percentual_geral) / 100,
                    1.0 / NULLIF(days_geral, 0)) - 1) * 100, 6)   As impacto_diario_geral,

       SIGN(crescimento_percentual_geral) *
       round((power(1 + ABS(crescimento_percentual_geral) / 100,
                    30.0 / NULLIF(days_geral, 0)) - 1) * 100, 6)  As impacto_mensalizado_geral,

       SIGN(crescimento_percentual_geral) *
       round((power(1 + ABS(crescimento_percentual_geral) / 100,
                    365.0 / NULLIF(days_geral, 0)) - 1) * 100, 6) As impacto_anualizado_geral,
...
select grupo_ativo                             as name,
       ...
       crescimento_percentual_patrimonio_geral as crescimento_percentual_geral
