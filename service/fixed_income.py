...
class FixedIncome:
    ...
    def add_stock_price(self, stock, headers, date=None):
        ...
        try:
            ...
            price_agg = self.remote_repository.get_object(
                "stocks_prices_agregated", data=filter, headers=headers
            )
            if price_agg is not None and price_agg["id"] is not None:
                ...
                self.remote_repository.insert(
                    "stocks_prices_agregated", stock_price, headers
                )
        except Exception as e:
    ...
    def get_stock_price(self, movement, headers=None):
        ...
        if type_ == "IPCA":
            ...
            date_mask = "%Y-%m-%d"
        date_price = datetime.strptime(
            price_obj["date_value"], "%Y-%m-%d %H:%M:%S.%f"
        ).strftime(date_mask)
