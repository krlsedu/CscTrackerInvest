...
class AttStocks:
    ...
    def att_acoes(self, headers=None, full=False, express=False):
        ...
        for acao in acoes:
            ...
            try:
                self.logger.info(
                    f"Atualizando a acao: {acao['ticker']} - {count_}/{len(acoes)}"
                )
                stock_ = self.investment_handler.get_stock(acao["ticker"], headers)

                if full:
                    stock_, investment_type, _ = (
                        self.http_repository.get_values_by_ticker(stock_, True, headers)
                    )
                else:
                    try:
    ...
    def att_criptos(self, headers=None, full=False):
        acoes = self.remote_repository.get_objects(
            "stocks", ["investment_type_id"], {"investment_type_id": 100}, headers
        )
        for acao in acoes:
            try:
    ...
    def att_brd_expres(self, headers=None):
        ...
        for bdr in bdrs:
            try:
                company_ = bdr["ticker"]
                stock_ = self.investment_handler.get_stock(company_, headers)
                stock_, investment_type, _ = self.http_repository.get_values_by_ticker(
                    stock_, True, headers
                )
                ...
                self.logger.info(
                    f"Erro ao atualizar {bdr['ticker']} - {bdr['name']} - {e}"
                )
        self.investment_handler.att_prices_list_yahoo(tickers_, headers)
        return bdrs
    ...
    def att_fiis(self, headers, full=False, express=False):
        ...
        for fii in fiis:
            count_ += 1
            self.logger.info(
                f"Atualizando o fundo: {fii['ticker']} - {count_}/{len(fiis)}"
            )
            try:
                ...
                self.logger.info(
                    "Erro ao atualizar o fundo: " + fii["ticker"] + " - " + str(e)
                )
        self.investment_handler.att_prices_list_yahoo(tickers_, headers)
        return fiis

    def att_fundos(self, headers=None):
        fundos = self.remote_repository.get_objects(
            "stocks", ["investment_type_id"], {"investment_type_id": 15}, headers
        )
        for fundo in fundos:
            try:
                stock_, investment_type, _ = self.http_repository.get_values_by_ticker(
                ...
                self.logger.info(
                    f"Erro ao atualizar {fundo['ticker']} - {fundo['name']} - {e}"
                )
        return fundos
    ...
    def att_dividends_info(self, headers):
        ...
        for stock in my_stocks:
            ...
            self.logger.info(
                f"Atualizando mapa de dividendos de {stock_['ticker']} - {stock_['name']} - "
    ...
    def att_user_dividends_info(self, headers):
        ...
        for stock in my_stocks:
            ...
            investment_type_id_ = (
                ...
                else stock_["investment_type_id"]
            )
            if (
                investment_type_id_ == 1
    ...
    def dividends_info(self, headers):
        ...
        for stock in my_stocks:
            try:
                ...
                self.logger.info(
                    f"Atualizando dividendos de {stock_['ticker']} - {stock_['name']} - "
                    f"{counter}/{len(my_stocks)}"
                )
                dividends_map = self.remote_repository.get_objects(
                    "dividends_map",
                    ["investment_id"],
                ...
                for dividend_map in dividends_map:
                    try:
                        ...
                        quantity = self.remote_repository.execute_select(
                            select, headers
                        )[0]["quantity"]
                        if quantity > 0:
                            dividend = {"dividends_map_id": dividend_map["id"]}
                            dividend = self.remote_repository.get_object(
                                "dividends", ["dividends_map_id"], dividend, headers
                            )
                            if dividend is None:
                                dividend = {
                                    "investment_id": stock_["id"],
                                ...
                                self.remote_repository.insert(
                                    "dividends", dividend, headers
                                )
                            else:
                                if (
                                    dividend["value_per_quote"] != value_per_quote
                                    or dividend["quantity"] != quantity
                                    or dividend["date_payment"]
                                    != dividend_map["date_payment"]
                                ):
                                    dividend["value_per_quote"] = value_per_quote
                                    dividend["quantity"] = quantity
                                    dividend["date_payment"] = dividend_map[
                                        "date_payment"
                                    ]
                                    self.remote_repository.update(
                                        "dividends", ["id"], dividend, headers
                                    )
                    except Exception as e:
                        self.logger.info(
                            "Erro ao atualizar dividendos de " + stock_["ticker"]
                        )
                        self.logger.exception(e)
                        pass
            except Exception as e:
            ...
            try:
                ...
                for row in rows:
                    ...
                    if finded:
                        if (
                            _type == "Rendimento"
                            or _type == "REND. TRIBUTADO"
                            or (_type == "Dividendos" and investment_type_id_ == 2)
                        ):
                            _type = 3
                        ...
                        dividend_map = self.remote_repository.get_object(
                            "dividends_map",
                            ["investment_id", "date_with", "date_payment", "type_id"],
                            dividend_map,
                            headers,
                        )
                        if dividend_map is None:
                            ...
                            self.remote_repository.insert(
                                "dividends_map", dividend_map, headers
                            )
                        else:
                            if dividend_map["value_per_quote"] != value:
                                dividend_map["value_per_quote"] = value
                                self.remote_repository.update(
                                    "dividends_map", ["id"], dividend_map, headers
                                )

            except Exception as e:
                self.logger.info(
                    "Erro ao inserir mapa de dividendos de " + stock_["ticker"]
                )
                self.logger.exception(e)
                pass
    ...
    def att_price_generic(self, headers, stock, type, daily=False):
        if type == "bdr":
            ...
            stock["price"] = stock_["price"]
        if type == "fundo":
            self.investment_handler.att_stock_price_new(
                headers, daily, stock, stock_, type
            )
        else:
    ...
    def att_indices(self, headers):
        ...
        for fii in fiis:
            count_ += 1
            self.logger.info(
                f"Atualizando o índice: {fii['ticker']} - {count_}/{len(fiis)}"
            )
            try:
                stock_ = self.investment_handler.get_stock(fii["ticker"], headers)
                if stock_["tx_type"] is None:
                    self.investment_handler.att_price_yahoo(stock_, headers)
                    stock_["price"] = self.http_repository.get_info(
                        stock_, "cotacao", headers
                    )
