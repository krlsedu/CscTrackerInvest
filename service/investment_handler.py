...
class InvestmentHandler:
    ...
    def add_movement(self, movement, headers=None):
        ...
        movement_type = self.remote_repository.get_object(
            "movement_types", ["id"], {"id": movement["movement_type"]}, headers
        )
        ...
        if ticker_ is not None:
            ...
            if fixed_icome != "S":
                ...
                try:
                    movement["quantity"] = (
                        float(movement["quantity"]) / movement["price"]
                    )
                except:
                    filter_ = {"investment_id": stock["id"]}
                    user_stock = self.remote_repository.get_object(
                        "user_stocks", data=filter_, headers=headers
                    )
            ...
            del movement["ticker"]
        try:
            if self.remote_repository.exist_by_key(
                "user_stocks", ["investment_id"], movement, headers
            ):
                if movement_type["to_balance"]:
                    ...
                    total_value = float(
                        user_stock["quantity"] * user_stock["avg_price"]
                    )

                    total_value += (
                        movement["quantity"] * movement["price"] * float(coef)
                    )
                    quantity = float(user_stock["quantity"]) + float(
                        movement["quantity"]
                    ) * float(coef)
                    if quantity > 0:
                    ...
                    profit_loss_value = float(movement["price"]) - float(
                    ...
                    self.remote_repository.update(
                        "user_stocks", ["user_id", "investment_id"], user_stock, headers
                    )
                try:
                    del movement["tx_type"]
                except:
                    pass
                self.remote_repository.insert(
                    "user_stocks_movements", movement, headers
                )
            else:
                if movement_type["to_balance"]:
                    stock = {
        ...
        try:
            try:
                stock = self.remote_repository.get_object(
                    "stocks", ["id"], {"id": movement["investment_id"]}, headers
                )
            except Exception as e:
                self.logger.exception(e)
                stock = {}
    ...
    def get_stock(self, ticker_, headers=None):
        ...
        stock = self.remote_repository.get_object(
            "stocks", ["ticker"], {"ticker": ticker_}, headers
        )
        try:
            stock["id"]
        except Exception as e:
    ...
    def add_price(self, price, headers=None):
        try:
            if price["price"] is not None and price["price"] != 0:
                ...
                try:
                    try:
                        date_value_ = datetime.strptime(
                            price["date_value"], "%Y-%m-%d"
                        ).strftime("%Y-%m-%d")
                    except:
                    ...
                    price_agg = self.remote_repository.get_object(
                        "stocks_prices_agregated", data=_filter, headers=headers
                    )
                    if price_agg is not None and price_agg["id"] is not None:
                        price_agg["price"] = price["price"]
                        self.remote_repository.update(
                            "stocks_prices_agregated", ["id"], price_agg, headers
                        )
                    else:
                        price["date_value"] = date_value_
                        self.remote_repository.insert(
                            "stocks_prices_agregated", price, headers
                        )
                except Exception as e:
    ...
    def get_stocks(self, args=None, headers=None):
        ...
        return self.remote_repository.get_objects(
            "user_stocks", filters, values, headers
        )
    ...
    def att_prices_list_yahoo(self, stocks_, headers, date=None):
        range_ = 50
        stocks_blocks = [
            stocks_[i : i + range_] for i in range(0, len(stocks_), range_)
        ]
                ...
                for key in prices.keys():
                    try:
                        ...
                        stock_ = self.remote_repository.get_object(
                            "stocks", data={"ticker": key_}, headers=headers
                        )
                        stock_["price"] = prices[key]["regularMarketPrice"]
                        self.add_stock_price(stock_, headers, date)
                        self.remote_repository.update(
                            "stocks", ["ticker"], stock_, headers
                        )
                    except Exception as e:
    ...
    def att_price_yahoo_us(self, stock_, headers, date=None):
        try:
            ...
            prices = self.http_repository.get(
                self.get_url_bff() + "yahoofinance/price/BRL=X", headers=headers
            ).json()
    ...
    def att_dividend_info(self, stock, headers):
        select = (
            f"select value_per_quote as value "
    ...
    def get_stocks_consolidated(self, args=None, headers=None):
        ...
        if stocks.__len__() > 0:
            ...
            for stock in stocks:
                ...
                if stock["quantity"] > 0:
                    ...
                    investment_type = self.remote_repository.get_object(
                        "stocks", ["id"], {"id": stock["investment_id"]}, headers
                    )
                    ticker_ = investment_type["ticker"]
                    stock["ticker"] = ticker_
                    stock_ = self.remote_repository.get_object(
                        "stocks", ["ticker"], stock, headers
                    )
                    stock_, investment_type, att_price_ = (
                        self.http_repository.get_values_by_ticker(
                            stock_, False, headers, 2
                        )
                    )
                    investment_type_id_ = (
                        stock_["investment_type_real_id"]
                        if stock_["investment_type_real_id"]
                        else investment_type["id"]
                    )
                    if att_price_:
                        if (
                            investment_type["id"] == 100
                    ...
                    if (
                        investment_type["id"] == 16
                        and stock_["investment_type_real_id"] is None
                    ):
                        stock_price = (
                            self.fixed_income_handler.get_stock_price_by_ticker(
                                ticker_, headers, (datetime.now()).strftime("%Y-%m-%d")
                            )
                        )
                        stock_["price"] = stock_price["price"]
                    stock_ = self.att_dividend_info(stock_, headers)
                    data_ant = (datetime.now() - timedelta(1)).strftime(
                        "%Y-%m-%d 23:59:59"
                    )
                    price_ant = self.stock_handler.get_price(
                        stock_["id"], data_ant, headers
                    )
                    data_atu = datetime.now().strftime("%Y-%m-%d 23:59:59")
                    price_atu = self.stock_handler.get_price(
                        stock_["id"], data_atu, headers
                    )
                    ...
                    if price_ant is not None:
                        ...
                        dt_prc_ant = (
                            datetime.strptime(
                                price_ant["date_value"], "%Y-%m-%d %H:%M:%S.%f"
                            )
                            .replace(tzinfo=timezone.utc)
                            .strftime("%Y-%m-%d")
                        )
                        dt_prc_req = (datetime.now() - timedelta(1)).strftime(
                            "%Y-%m-%d"
                        )
                        if dt_prc_ant < dt_prc_req:
                            if investment_type_id_ == 15:
                                self.att_stock_price_new(
                                    headers,
                                    False,
                                    stock_,
                                    stock_,
                    ...
                    if (
                        ...
                        perc_gain = (
                            ...
                            / float(stock_consolidated["avg_price"])
                            - 1
                        )

                        stock_consolidated["total_value_atu"] = float(
                            stock_consolidated["total_value_invest"]
                        ) + float(
                            perc_gain * float(stock_consolidated["total_value_invest"])
                        )

                        if price_ant is not None:
                            perc_gain_ant = (
                                float(price_ant["price"])
                                / float(stock_consolidated["avg_price"])
                            ) - 1

                            stock_consolidated["total_value_ant"] = float(
                                stock_consolidated["total_value_invest"]
                            ) + float(
                                perc_gain_ant
                                * float(stock_consolidated["total_value_invest"])
                            )

                            stock_consolidated["value_ant_date"] = (
                                datetime.strptime(
                                    price_ant["date_value"], "%Y-%m-%d %H:%M:%S.%f"
                                )
                                .replace(tzinfo=timezone.utc)
                            ...
                            stock_consolidated["variation"] = (
                                stock_consolidated["total_value_atu"]
                                - stock_consolidated["total_value_ant"]
                            )
                            stock_consolidated["variation_perc"] = (
                                stock_consolidated["total_value_atu"]
                                / stock_consolidated["total_value_ant"]
                                - 1
                            )
                    ...
                    stock_consolidated["gain"] = (
                        float(stock_consolidated["total_value_atu"])
                        / float(stock_consolidated["total_value_invest"])
                        - 1
                    )
                    infos_ = stock_["url_infos"]
                    if infos_ is not None:
                        stock_consolidated["url_statusinvest"] = (
                            "https://statusinvest.com.br" + infos_
                        )

                    stock_consolidated = self.add_dividend_info(
                        stock_consolidated, headers
                    )
                    stock_consolidated = self.add_daily_gain(
                        stock_consolidated, headers
                    )
                    ...
                    segment["total_value_invest"] = float(
                        stock_consolidated["total_value_invest"]
                    )
                    segment["total_value_atu"] = stock_consolidated["total_value_atu"]

                    if price_ant is not None:
                        segment["total_value_ant"] = stock_consolidated[
                            "total_value_ant"
                        ]
            ...
            ret = {"stocks": json.loads(df_stocks.to_json(orient="records"))}
            ...
            ret["segments_grouped"] = json.loads(
                df_grouped_segment.to_json(orient="table")
            )["data"]
            ret["type_grouped"] = json.loads(df_grouped_type.to_json(orient="table"))[
                "data"
            ]
    ...
    def buy_sell_indication(self, args=None, headers=None):
        ...
        stocks_br, bdrs, fiis, founds, fix_income, criptos = self.get_sotcks_infos(
            args, headers
        )
        stock_ref = None
        for stock in stocks:
            ...
            stock_ref = None
            stock_ref = self.get_stock_ref(
                bdrs, fiis, stock_, stock_ref, stocks_br, founds, fix_income, criptos
            )
    ...
    def save_recomendations(self, recomendations, headers=None):
        for recomendation in recomendations:
            recomendation["investment_id"] = self.remote_repository.get_object(
                "stocks", ["ticker"], recomendation, headers
            )["id"]
            try:
            ...
            try:
                self.remote_repository.insert(
                    "user_recomendations", recomendation, headers
                )
            except Exception as e:
                self.logger.exception(e)
                pass
    ...
    def save_infos(self, infos, headers=None):
        resume = None
        try:
            resume = self.remote_repository.get_object(
                "investments_calc_resume", [], {}, headers
            )
        except:
            pass
    ...
    def add_total_dividends_info(self, infos, headers=None):
        ...
        for dividend in dividends:
            infos["resume"]["total_dividends"] += float(
                dividend["quantity"] * dividend["value_per_quote"]
            )
        infos["resume"]["dyr"] = (
            infos["resume"]["total_dividends"] / infos["resume"]["total_value_atu"]
        )
        return infos

    def add_total_profit_loss_info(self, infos, headers=None, args=None):
        profit_loss = self.remote_repository.get_objects(
            "profit_loss", data=args, headers=headers
        )
    ...
    def add_total_daily_gain(self, infos, headers=None):
        ...
        movements = self.remote_repository.get_objects(
            "user_stocks_movements", data=filter_, headers=headers
        )
        days = 0
        quantity = 0
        for movement in movements:
            date_ = datetime.strptime(movement["date"], "%Y-%m-%d %H:%M:%S.%f").replace(
                tzinfo=timezone.utc
            )
        ...
        if avg_days is None or avg_days < 1:
            avg_days = 1
        infos["resume"]["gain"] = (
            infos["resume"]["total_value_atu"] / infos["resume"]["total_value_invest"]
            - 1
        )
        infos["resume"]["daily_gain"] = (
            self.root_n(infos["resume"]["gain"] + 1, avg_days) - 1
        )
        infos["resume"]["daily_dyr"] = (
            self.root_n(infos["resume"]["dyr"] + 1, avg_days) - 1
        )
        profits_r = (infos["resume"]["profits"] - infos["resume"]["losses"]) / infos[
            "resume"
        ]["total_value_atu"]
        ...
        infos["resume"]["monthly_gain"] = (
            infos["resume"]["daily_total_gain"] + float(1)
        ) ** float(30) - float(1)
    ...
    def add_daily_gain(self, stock, headers=None):
        ...
        buy_movements = self.remote_repository.get_objects(
            "user_stocks_movements", data=filter_, headers=headers
        )
        filter_["movement_type"] = 2
        sell_movements = self.remote_repository.get_objects(
            "user_stocks_movements", data=filter_, headers=headers
        )
        days = 0
        ...
        stock["monthly_gain"] = (stock["daily_total_gain"] + float(1)) ** float(
            30
        ) - float(1)
    ...
    def set_stock_weight(self, stock, types_sum, stocks):
        ...
        try:
            ...
            stock["segment_weight_in_type"] = (
                segment_total / types_sum[stock["investment_type_id"]]
            )
        except:
        ...
        try:
            stock["ticker_weight_in_type"] = (
                stock["total_value_atu"] / types_sum[stock["investment_type_id"]]
            )
        except:
    ...
    def check_notify_stock(self, stock, headers=None):
        ...
        stock_notification_config = self.remote_repository.get_object(
            "stock_notification_config", ["investment_id"], filter_, headers
        )
        if stock_notification_config is None:
            ...
            if (
                stock_notification_config["monthly_gain_target"] is not None
                and stock_notification_config["monthly_gain_target"] / 100
                < stock["monthly_gain"]
                and self.check_time_to_notfy(stock_notification_config, headers)
            ):
                message = (
                    "A ação "
                    + stock["ticker"]
                    + " atingiu o ganho mensal esperado de "
                    + str(stock_notification_config["monthly_gain_target"])
                    + "%. Ganho atual: "
                ...
                stock_notification_config["last_notification"] = datetime.strftime(
                    datetime.now(pytz.utc), "%Y-%m-%d %H:%M:%S.%f"
                )
                self.remote_repository.update(
                ...
                stock_notification_config["value_target"] is not None
                and stock_notification_config["value_target"] < stock["price_atu"]
                and self.check_time_to_notfy(stock_notification_config, headers)
            ):
                message = (
                    "A ação "
                    + stock["ticker"]
                    + " atingiu o valor desejado de "
                    + str(stock_notification_config["value_target"])
                    + ". Valor atual: "
                    + str(stock["price_atu"])
                )
                Utils.inform_to_client(
    ...
    def check_time_to_notfy(self, stock_notification_config, headers=None):
        try:
            last_update = datetime.strptime(
                stock_notification_config["last_notification"], "%Y-%m-%d %H:%M:%S.%f"
            ).replace(tzinfo=timezone.utc)
    ...
    def set_buy_sell_info(
        self, stock_, stock_ref, types_sum, stocks, headers=None, notify=False
    ):
        ...
        stock_["ticker_weight_ideal"] = self.get_ticker_weight_ideal(
            stock_["ticker"], headers
        )
        if stock_ref is not None:
            try:
                if stock_[perc_refer] is None:
                    stock_[perc_refer] = self.get_ticket_info(
                        stock_["ticker"], stocks, perc_refer
                    )
            except:
                stock_[perc_refer] = self.get_ticket_info(
                    stock_["ticker"], stocks, perc_refer
            ...
            try:
                if stock_["monthly_gain"] is None:
                    stock_["monthly_gain"] = self.get_ticket_info(
                        stock_["ticker"], stocks, "monthly_gain"
                    )
            except:
                stock_["monthly_gain"] = self.get_ticket_info(
                    stock_["ticker"], stocks, "monthly_gain"
            ...
            if rank <= max_rank_to_buy:
                if (
                    stock_["monthly_gain"] > great_gain
                    ...
                    stock_["recommendation"] = "Manter posição atual -> Rank: " + str(
                        rank
                    )
            elif rank > 40:
                if stock_["monthly_gain"] > great_gain:
                    ...
                    stock_["recommendation"] = (
                        ...
                        + self.to_percent_from_aliq(stock_["monthly_gain"])
                        + " -> Vender "
                        + self.to_brl(types_sum[0] * stock_["ticker_weight_in_all"])
                        + " -> Rank: "
                        + str(rank)
                    )
                    ...
                    stock_["recommendation"] = (
                        "Manter posição atual (Período de carência x3) -> Rank: "
                        + str(rank)
                    )
                else:
                    stock_["buy_sell_indicator"] = "sell"
                ...
                elif self.is_not_in_ideal_perc(stock_, types_sum) and stock_[
                    "avg_days"
                ] > (min_days_to_sell * 2):
                    stock_["buy_sell_indicator"] = "sell"
                    stock_["recommendation"] = (
            ...
            if stock_["monthly_gain"] > great_gain:
                ...
                stock_["recommendation"] = (
                    "Manter posição atual (Período de carência) "
                    "-> Não ranqueada nas recomendações"
                )
            else:
                stock_["buy_sell_indicator"] = "sell"
                stock_["recommendation"] = (
                    "Não comprar -> Não ranqueada nas recomendações"
                )

    def is_not_in_ideal_perc(self, stock_, types_sum=None, buy=False):
        if buy:
            is_not_ideal_ = (
                stock_["ticker_weight_in_all"] < stock_["ticker_weight_ideal"]
            )
        else:
            is_not_ideal_ = (
                stock_["ticker_weight_in_all"] > stock_["ticker_weight_ideal"]
        ...
        if is_not_ideal_ and types_sum is not None:
            if buy:
                value = self.get_tot_to_buy(
                    types_sum[0],
                    stock_["ticker_weight_ideal"],
                    stock_["ticker_weight_in_all"],
                )
            else:
                value = self.get_tot_to_sell(
                    types_sum[0],
                    stock_["ticker_weight_ideal"],
                    stock_["ticker_weight_in_all"],
                )
    ...
    def get_stock_ref(
        self, bdrs, fiis, stock, stock_ref, stocks_br, founds, fix_income, criptos
    ):
        ...
        if stock_["prices_imported"] == "N" or daily or reimport:
            if type == "fundo" and not daily or type == "fundo" and reimport:
                try:
                    ...
                    infos = self.http_repository.get_prices_fundos(
                        company_, price_type == "1"
                    )
                    ...
                    self.logger.error(
                        f"Error importing prices for {stock_['ticker']}: {e}"
                    )
                pass
            else:
                infos = self.http_repository.get_prices(
                    stock_["ticker"], type, daily, price_type
                )
                for info in infos:
                    prices = info["prices"]
                    for price in prices:
    ...
    def investment_facts(self, facts, headers=None):
        ...
        for fact in facts:
            ...
            try:
                fact["id"]
                fact_ = self.remote_repository.get_object(
                    "user_invest_facts", ["id"], fact, headers
                )
            except KeyError:
                fact["id"] = None
            ...
            user_invest_configs = self.remote_repository.get_object(
                "user_invest_configs", ["investment_id"], filter, headers
            )
            if user_invest_configs is None:
                ...
                self.remote_repository.insert(
                    "user_invest_configs", user_invest_configs, headers
                )
            ...
            self.remote_repository.update(
                "user_invest_configs", ["investment_id"], user_invest_configs, headers
            )
    ...
    def get_investment_facts(self, ticker, headers=None):
        ...
        facts = self.remote_repository.get_objects(
            "user_invest_facts", ["investment_id"], filter, headers
        )
        for fact in facts:
    ...
    def investment_calc(self, data, headers=None):
        ...
        select = (
            f"select * from user_invest_apply where user_id = :user_id "
            f" order by apply_date desc limit 1"
        )
        user_invest_apply = self.remote_repository.execute_select(select, headers)[0]
        perc_ideal_investment_types = self.remote_repository.get_all_objects(
            "perc_ideal_investment_type", headers
        )
        amount_types = []
        ...
        for investment_type in perc_ideal_investment_types:
            ...
            type_gruped_values = self.remote_repository.get_object(
                "type_gruped_values",
                ["type_id"],
            ...
            amount = perc_ideal * (
                resume["total_value_atu"] + user_invest_apply["amount"]
            )
            amount = amount - amount_atu
            if amount > 100:
                total_amount = total_amount + amount
                amount_type = {
                    "investment_type_id": investment_type["investment_type_id"],
                    "amount": amount,
        ...
        for amount_type in amount_types:
            ...
            if amount > 100:
                ...
                self.remote_repository.insert(
                    "user_invest_apply_type", amount_type, headers
                )
            ...
            if investment_type_id == 16:
                amount = amount_type["amount"]
                stock_ = self.remote_repository.get_object(
                    "stocks", data={"ticker": "Renda fixa"}, headers=headers
                )
                total_amount_stock = total_amount_stock + amount
                amount_stocks.append(
                    {
                        "investment_id": stock_["id"],
                        "amount": amount,
                        "user_invest_apply_id": user_invest_apply["id"],
                                    ...
                                    and (rank <= {config_['rank']} \
                                            or investment_id in (select us.investment_id from user_stocks us \
                                                                where us.user_id = :user_id  \
                                                                    ...
                                                                    and us.quantity >0)) \
                         order by rank"
                user_recomendations = self.remote_repository.execute_select(
                    select, headers
                )
                for user_recomendation in user_recomendations:
                    amount, stock_ = self.get_amount_value(
                        user_recomendation,
                        resume,
                        headers,
                        config_["rank"],
                    ...
                    if (
                        amount > stock_["price"] or stock_["investment_type_id"] == 100
                    ) and valid:
                        total_amount_stock = total_amount_stock + amount
                        amount_stocks.append(
                            {
            ...
            for amount_stock in amount_stocks:
                stock_ = self.remote_repository.get_object(
                    "stocks", ["id"], {"id": amount_stock["investment_id"]}, headers
                )
                amount = amount_stock["amount"]
                if amount > stock_["price"] or stock_["investment_type_id"] == 100:
                    amount_stock["amount"] = amount_type["amount"] * (
                        amount / total_amount_stock
                    )
                    if amount_stock["amount"] > amount:
                        amount_stock["amount"] = amount
                    if (
                        amount_stock["amount"] > stock_["price"]
                        or stock_["investment_type_id"] == 100
                    ):
            ...
            if amount_ajsut > 0:
                for ajsut in amount_stocks_ajust:
                    ...
                    stock_ = self.remote_repository.get_object(
                        "stocks", ["id"], {"id": ajsut["investment_id"]}, headers
                    )
                    ajsut["amount"] = amount_type["amount"] * (
                        ajsut["amount"] / amount_ajsut
                    )
                    ...
                    self.remote_repository.insert(
                        "user_invest_apply_stock", ajsut, headers
                    )
                ...
                for user_recomendation in user_recomendations:
                    ...
                    if resto > stock_["price"] and amount > 0 and valid:
                        ...
                        if user_invest_apply_stock is None:
                            ...
                            self.remote_repository.insert(
                                "user_invest_apply_stock", amount_stock, headers
                            )
                            user_invest_apply_stock = self.remote_repository.get_object(
                                "user_invest_apply_stock",
                                ["investment_id", "user_invest_apply_id"],
                                {
                                    "investment_id": user_recomendation[
                                        "investment_id"
                                    ],
                                    "user_invest_apply_id": user_invest_apply["id"],
                                },
                        ...
                        if user_invest_apply_stock["amount"] < amount:
                            user_invest_apply_stock["amount"] = (
                                user_invest_apply_stock["amount"] + stock_["price"]
                            )
                            user_invest_apply_stock["num_quotas"] = (
                                user_invest_apply_stock["num_quotas"] + 1
                            )
                            self.remote_repository.update(
                                "user_invest_apply_stock",
                                ["id"],
        ...
        if resto_total > 0:
            ...
            if user_invest_apply_stock is None:
                ...
                user_invest_apply_stock["amount"] = (
                    user_invest_apply_stock["amount"] + resto_total
                )
                user_invest_apply_stock["num_quotas"] = (
                    user_invest_apply_stock["num_quotas"] + resto_total
                )
                self.remote_repository.update(
                    "user_invest_apply_stock", ["id"], user_invest_apply_stock, headers
                )
    ...
    def get_amount_value(
        self, user_recomendation, resume, headers, rank=20, aport_value=0
    ):
        stock_ = self.remote_repository.get_object(
            "stocks", ["id"], {"id": user_recomendation["investment_id"]}, headers
        )
        perc_ideal = self.get_ticker_weight_ideal(stock_["ticker"], headers, rank)

        user_stock = self.remote_repository.get_object(
            "user_stocks", ["investment_id"], user_recomendation, headers
        )
        if stock_["segment_custom"] is not None:
    ...
    def get_sell_sugestions(self, user_invest_apply, headers=None):
        ...
        user_stocks = [
            user_stock for user_stock in user_stocks if user_stock["quantity"] > 0
        ]
        for user_stock in user_stocks:
            stock_ = self.remote_repository.get_object(
                "stocks", ["id"], {"id": user_stock["investment_id"]}, headers
            )
            user_recommendation = self.remote_repository.get_object(
                "user_recomendations", ["investment_id"], user_stock, headers
            )
            ...
            if user_recommendation is not None:
                if (
                    user_recommendation["buy_sell_indicator"] == "sell"
                    or user_recommendation["buy_sell_indicator"] == "great-gain"
                ):
                    if user_recommendation["rank"] > 20:
                        user_invest_sell_stock["motive"] = (
                            user_recommendation["buy_sell_indicator"]
                            + " - "
                            + str(user_recommendation["rank"])
                        )
                        self.remote_repository.insert(
                            "user_invest_sell_stock", user_invest_sell_stock, headers
                        )
            else:
    ...
    def last_investment_calc(self, headers=None):
        ...
        select = (
            f"select * from user_invest_apply where user_id = {user_['id']}"
            f" order by apply_date desc limit 1"
        )
        user_invest_apply = self.remote_repository.execute_select(select, headers)[0]
    ...
    def investment_calc_info(self, user_invest_apply, headers):
        ...
        for stock_recomendation in stock_recomendations:
            stock_ = self.remote_repository.get_object(
                "stocks", ["id"], {"id": stock_recomendation["investment_id"]}, headers
            )
            ...
            if user_recomendation_ is None:
                user_recomendation_ = {"rank": 1}
            user_stock_ = self.remote_repository.get_object(
                "user_stocks", data={"investment_id": stock_["id"]}, headers=headers
            )
            if user_stock_ is None:
                user_stock_ = {"quantity": 0}
            ...
            stock_recomendation_ = {
                ...
                "value_or_cotas": 0,
                "tax_or_price": 0,
                "avg_value_quota_invested": stock_recomendation[
                    "avg_value_quota_invested"
                ],
    ...
    def save_aplly_stock(self, apply_stock, headers):
        ...
        user_invest_apply_stock = self.remote_repository.get_object(
            "user_invest_apply_stock", ["id"], apply_stock, headers
        )
        if (
            user_invest_apply_stock is not None
            and user_invest_apply_stock["invested"] == "N"
            and apply_stock["tax_or_price"] > 0
            and apply_stock["value_or_cotas"] > 0
            ...
            user_invest_apply_type_ = self.remote_repository.get_object(
                ...
                data={
                    "investment_type_id": stock_["investment_type_id"],
                    "user_invest_apply_id": user_invest_apply_stock[
                        "user_invest_apply_id"
                    ],
                },
                headers=headers,
            ...
            if (
                ...
                user_invest_apply["value_invested"] = (
                    user_invest_apply["value_invested"] + apply_stock["value_or_cotas"]
                )

                user_invest_apply_type_["value_invested"] = (
                    user_invest_apply_type_["value_invested"]
                ...
                avg_value_quota_invested_ += (
                    apply_stock["value_or_cotas"] * apply_stock["tax_or_price"]
                )
                user_invest_apply_stock["num_quotas_invested"] += apply_stock[
                    "value_or_cotas"
                ]
                user_invest_apply_stock["avg_value_quota_invested"] = (
                    avg_value_quota_invested_
                ...
                user_invest_apply["value_invested"] = user_invest_apply[
                    "value_invested"
                ] + (apply_stock["value_or_cotas"] * apply_stock["tax_or_price"])
                movement = {
                    "movement_type": 1,
                ...
                user_invest_apply_type_["value_invested"] = user_invest_apply_type_[
                    "value_invested"
                ] + (apply_stock["value_or_cotas"] * apply_stock["tax_or_price"])
                ...
                if diff_ < user_invest_apply_stock["avg_value_quota_invested"]:
                    ...
                    user_invest_apply_stock_rf["amount"] = (
                        user_invest_apply_stock_rf["amount"] + diff_
                    )
                    user_invest_apply_stock_rf["num_quotas"] = (
                        user_invest_apply_stock_rf["num_quotas"] + diff_
                    )
                    self.remote_repository.update(
                        "user_invest_apply_stock",
                        ["id"],
            ...
            self.remote_repository.update(
                "user_invest_apply", ["id"], user_invest_apply, headers
            )
            self.remote_repository.update(
                "user_invest_apply_type", ["id"], user_invest_apply_type_, headers
            )
            ...
            self.remote_repository.update(
                "user_invest_apply_stock", ["id"], user_invest_apply_stock_rf, headers
            )
            ...
            added_ = {
                "status": "Cuidado",
                "message": "Registro sem informação de investimento",
            }
    ...
    def save_aplly_stock_sell(self, apply_stock, headers):
        user_invest_sell_stock = self.remote_repository.get_object(
            "user_invest_sell_stock", ["id"], apply_stock, headers
        )
        if (
            user_invest_sell_stock is not None
            and user_invest_sell_stock["executed"] == "N"
        ):
            ...
            self.remote_repository.update(
                "user_invest_sell_stock", ["id"], user_invest_sell_stock, headers
            )
            movement = {
                "movement_type": 2,
                "ticker": apply_stock["ticker"],
    ...
    def add_resumes_period(self, args, headers):
        ...
        for tipo in tipos:
            ...
            if args_["refazer_data_fim"] == "S":
                ...
                Utils.inform_to_client(
                    args_, "Resumo por período solicitado", headers, msg_
                )
                for date in date_range:
                    ...
                    if (
                        date.strftime("%Y-%m-%d") > data_ini_
                        and date.strftime("%Y-%m-%d") < data_fim_
                    ):
            ...
            self.remote_repository.insert(
                "user_resume_calculed", resume_calculed, headers
            )
    ...
    def get_resume_invest_grafic(self, args, headers):
        ...
        result_ = {"categories": dates, "series": series}
        return result_
    ...
    def load_prices(self, ticker, type, name=None, headers=None, data_=None):
        ...
        for info in infos:
            ...
            for price in prices:
                stock_["price"] = price["price"]
                data = datetime.strptime(price["date"], "%d/%m/%y %H:%M").strftime(
                    "%Y-%m-%d"
                )
                can_insert = True
                if data_ is not None:
